...
import com.mycompany.progetto_moto.Pilota;
...
public class Ordinatore 
{
     ...
     * @param pos2 
     */
    public static void scambia(Pilota[] v, int pos1,int pos2)
    {
        Pilota sost;
        sost=v[pos1];
        v[pos1]=v[pos2];
     ...
     * @return il vettore in ordine crescente
     */
    public static Pilota[] ordinaPilotaCrescente(Pilota[] v)
    {
        int lunghezza=v.length;
        //Creo una copia di v e lo chiamo vOrdinato
        Pilota[] vOrdinato=new Pilota[lunghezza];
        for(int i=0;i<lunghezza;i++)
            vOrdinato[i]=v[i];
        ...
        for(int i=0;i<lunghezza-1;i++)
        {
            for(int j=i+1;j<lunghezza;j++)
            {
                if (vOrdinato[i].getCognome().compareToIgnoreCase(vOrdinato[j].getCognome())>0)
                    scambia(vOrdinato, i, j);
            }
        }
       return vOrdinato;
